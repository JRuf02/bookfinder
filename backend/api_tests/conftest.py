...
def app() -> Generator[Flask, None, None]:
    """Create a Flask app instance for testing, with no mocking of external servers.
    If this fixture is used, the app will make calls to the real DNB server,
    so it will only work if the DNB server is up.
    """
...
def client(app: Flask) -> FlaskClient:
    ...
    If a test produces a call to the DNB API, the real DNB server will be called.
    Tests that require calls to the DNB API will fail if the DNB server is down.
    """
...
@pytest.fixture
def _mock_dnb_api(requests_mock: Mocker) -> None:
    ...
    Mocks the DNB (Deutsche Nationalbibliothek) API responses.
    """
    ...
    path = Path(__file__).parent / "assets" / "978-3-551-35401-3-L.jpeg"
    with path.open("rb") as f:
        image_bytes_l = f.read()

    path = Path(__file__).parent / "assets" / "978-3-551-35401-3-M.jpeg"
    with path.open("rb") as f:
        image_bytes_m = f.read()

    path = Path(__file__).parent / "assets" / "978-3-551-35401-3-S.jpeg"
    with path.open("rb") as f:
...
@pytest.fixture
def mocked_app(app: Flask, _mock_dnb_api: None) -> Flask:
...
@pytest.fixture
def mocked_client(mocked_app: Flask) -> FlaskClient:
    """Create a Flask test client for testing the Flask app instance.
    Any calls to the DNB API (which is an external server) are mocked,
    to ensure testing our backend logic works even if the DNB server is down.
    """
